...
Likelihood<Spatial, Prior>::Likelihood (const Problem& data, const Parameters<Prior>& par) 
  ...
  , dl_dsigma (data.n_popul, data.n_loci, arma::fill::zeros)
  , dl_dmu (data.n_popul, data.n_loci, arma::fill::zeros)
  , dlp_dC (dl_dC)
  , dlp_dsigma (dl_dsigma)
  , dlp_dmu (dl_dmu)
{
  if (!singular)
...
{
  mat out;
  singular = !arma::inv_sympd(out, cov.C + data.X * par.LLt * data.X.t() + arma::diagmat(sigma));
  return out;
}
...
void Likelihood<Spatial, Prior>::fit_field (const uword i)
{
  ...
  Field field (data.Y.col(i), data.N.col(i), mu, Q);

  ll(i) = field.lapapp; 

  dlp_dC.col(i) = arma::vectorise(field.dlp_dC);
  dlp_dsigma.col(i) = field.dlp_dC.diag();
...
vec Likelihood<Spatial, Prior>::gradient (const Parameters<Prior>& par) const
{
  ...
  vec grad_C = arma::mean(dlp_dC, 1),
      grad_s = arma::mean(dlp_dsigma, 1),
      grad_mu = arma::mean(dlp_dmu, 1);

  // let s = exp(2 * S), S = Z * z
  // by the chain rule, we have
  // dlp/dS = dlp/ds % d(exp(2 * S))/dS = dlp/ds % 2 * exp(2 * S) = dlp/ds % 2 * s
...
mat Likelihood<Spatial, Prior>::fisher (const Parameters<Prior>& par) const
{
  ...
  mat dlp_dt = cov.dC_dt.t() * dlp_dC,
      ...
      dlp_ds = dlp_dsigma.each_col() % (2.*sigma), // see ::gradient above
  ...
  // FIXME
  // rather than this eigendecomposition, could use a modified Cholesky decomposition
  // and return the Cholesky factor
  mat eigvec;
  vec eigval;
